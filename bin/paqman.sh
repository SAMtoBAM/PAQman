...
[[ ! "$window" =~ ^[0-9]+$ ]] && echo "ERROR: --window option need to be a number (bp)" && exit
[[ ! "$slide" =~ ^[0-9]+$ ]] && echo "ERROR: --slide option need to be a number (bp)" && exit
[[ ! "$coveragemax" =~ ^[0-9]+$ ]] && echo "ERROR: --coveragemax option need to be a number" && exit
[[ ! "$sortmem" =~ ^[0-9]+[KMG]$ ]] && echo "ERROR: --sortmem option must be an non-decimal integer ending with K, M, or G" && exit
[[ ! "$merylmem"  =~ ^[0-9]+([.][0-9]+)?$ ]] && echo "ERROR: --merylmem option need to be a number (GB)" && exit
[[ ! "$merylkmer" =~ ^[0-9]+$ ]] && echo "ERROR: --merylkmer option need to be a number" && exit
...
##set alternative threads for samtools sorting
##-@ parameter is additional threads to use above one so just setting threads-1
threadssort=$((threads - 1))
...
[[ $platform == "ont" ]] && minimap2 --secondary=no -ax map-ont -t ${threads} ${assembly} longreads.rasusa.fq.gz | samtools sort -@ ${threadssort} -m ${sortmem} -o ${prefix}.minimap.sorted.bam -
[[ $platform == "pacbio-hifi" ]] && minimap2 --secondary=no -ax map-hifi -t ${threads} ${assembly} longreads.rasusa.fq.gz | samtools sort -@ ${threadssort} -m ${sortmem} -o ${prefix}.minimap.sorted.bam -
[[ $platform == "pacbio-clr" ]] && minimap2 --secondary=no -ax map-pb -t ${threads} ${assembly} longreads.rasusa.fq.gz | samtools sort -@ ${threadssort} -m ${sortmem} -o ${prefix}.minimap.sorted.bam -
...
#bwa mem -t ${threads} ${assembly} ${pair12} ${pair22} | samtools sort -@ ${threadssort} -m ${sortmem} -o ${prefix}.bwamem.sorted.bam -
bwa mem -t ${threads} ${assembly} ${pair12} ${pair22} | samtools sort -@ ${threadssort} -m ${sortmem} -o ${prefix}.bwamem.sorted.bam
...
seqkit locate -j ${threads} --ignore-case -r -p "\"${telomererepeat}\"" ${assembly} | tail -n+2 | awk '{print $1"\t"$5"\t"$6"\t"$4}' | sort -k1,1 -k2,2n | bedtools merge -d ${repeatsize} -c 4 -o distinct -i - | awk -v minrepeatsize="$minrepeatsize" '{if($3-$2 > minrepeatsize) print}' | awk -F "," '{print $1}' >> ./telomerality/telomeres.bed
...
t2t=$(awk '$5=="telomeric"{c[$1]++} END{for (i in c) if (c[i]==2) n++; print n+0}' ./telomerality/telomeres.classification.tsv)
##combine the two
