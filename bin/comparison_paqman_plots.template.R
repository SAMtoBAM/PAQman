...
names(comparisonsrad) = gsub(pattern = "_", replacement = " ", x = names(comparisonsrad))
names(comparisonsrad) = gsub(pattern = "\\(", replacement = " \\(", x = names(comparisonsrad))
...
comparisonsdot=subset(comparisons, select = c("label", "quast_#contigs", "quast_#contigs>10kb", "quast_assembly_size", "quast_assembly_N50", "quast_assembly_N90", "quast_largest_contig", "merqury_qv(phred)", "telomeric_ends", "T2T_contigs"))
...
ggsave(filename = "PATHTOOUTPUT.raw_values.svg", plot = absplot3,
       width = 350, height = 175, units = "mm", dpi = "retina")
