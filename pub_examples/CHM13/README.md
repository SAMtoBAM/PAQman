...
### 2.B Other publically available CHM13 assembly generated using long-reads
    ...
    cat ncbi_dataset/data/$genome/$genome*.fna | pigz -p ${threads} > assemblies/${genome2}.fa
    done
...
### 2.A PacBio HiFi reads
    ...
    cat *.fastq | pigz -p ${threads} > reads/pacbio/SRR9087XXX.pacbio.fq.gz
    ##remove uncompressed
    rm *.fastq

    ##get stats on the dataset quickly
    longreadsum fq -t ${threads} -i reads/pacbio/SRR9087XXX.pacbio.fq.gz -o pacbio/SRR9087XXX.stats
...
### 2.B ONT reads NOT USING
    ...
    wget -qO- https://s3-us-west-2.amazonaws.com/human-pangenomics/T2T/CHM13/nanopore/rel8-guppy-5.0.7/reads.fastq.gz | gunzip -c | seqkit seq -m 7500 | seqkit head -l 75000000000 | pigz -p ${threads} > reads/ont/rel8.25X_subset.ont.fq.gz
